...
namespace nonstd {
	...
	template<typename R>
	class CachedFunction<R> {
		std::function<R()> func_;
		mutable std::unordered_map<std::tuple<>, R> cache_; // Use an empty tuple for key
		mutable std::unordered_map<std::tuple<>, std::chrono::steady_clock::time_point> expiry_;
		DWORD cacheTime_ = CacheNormalTTL;
		R callFunctionAndCache() const {
			auto now = std::chrono::steady_clock::now();
			std::tuple<> argsTuple; // Empty tuple represents no arguments
	...
	public:
		explicit CachedFunction(const std::function<R()>& func, DWORD cacheTime = CacheNormalTTL) : func_(func), cacheTime_(cacheTime) {}
		R operator()() const {
